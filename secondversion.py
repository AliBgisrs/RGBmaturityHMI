...
# ---------------- FastAPI ----------------
app = FastAPI(title="HMI Web App", version="2.7.0")
app.add_middleware(
    CORSMiddleware,
...
def tpmax_tpmin_pchip(dap_arr: np.ndarray, exg_arr: np.ndarray) -> Tuple[int, float, int, float, np.ndarray, np.ndarray]:
    """
    Robust TPmax/TPmin using monotone PCHIP (no overshoot).
    1) Fit PCHIP on observed (x,y).
    2) Find interior maxima via derivative sign change (+ -> -).
    3) Choose highest max as TPmax; TPmin is min after TPmax on smoothed curve.
    4) Map both to nearest observed DAP.
    """
    ...
    # candidates where sign goes + -> -
    candidates = []
    for i in range(1, len(xs)-1):
        if sign[i-1] > 0 and sign[i+1] < 0:
            candidates.append(i)

    # pick highest candidate (by ys)
    if candidates:
        i_star = int(sorted(candidates, key=lambda i: ys[i], reverse=True)[0])
        tpmax_est = xs[i_star]
    else:
        # fallback: global maximum on smoothed curve
        tpmax_est = float(xs[np.nanargmax(ys)])

    # TPmin after TPmax on smoothed curve
    after = xs >= tpmax_est
    if np.any(after):
